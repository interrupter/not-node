...
describe("filter parser", function() {
  ...
  describe("filterSearch input", function() {
    ...
    it("filterSearch string", function() {
        var searchFor = "John";
        var filter   = query.getFilter({filterSearch: searchFor}, modelSchema);
        expect(filter).to.deep.equal([{name: new RegExp('.*'+escapeStringRegexp(searchFor)+'.*', 'i')}]);
    });
    ...
    it("filter by number field", function() {
        var searchFor = 12;
        var filter   = query.getFilter({id: searchFor}, modelSchema);
        expect(filter).to.deep.equal([
            {
                id: 12
            }
    ...
    it("filter by string field", function() {
        var searchFor = "John";
        var filter   = query.getFilter({name: searchFor}, modelSchema);
        expect(filter).to.deep.equal([
            {
                name: searchFor
            }
...
describe("sorter parser", function() {
  ...
  describe("not empty input, but field forbiden to sort", function() {
    it("parse", function() {
        var sorter = query.getSorter({sortDirection: -1, sortByField: "name"}, modelSchema);
  ...
  describe("not empty input, with sorter defaults", function() {
    it("full input", function() {
        var sorter = query.getSorter({sortDirection: 1, sortByField: "country"}, modelSchema, sorterDefaults);
        expect(sorter).to.deep.equal({"country": 1});
    });
