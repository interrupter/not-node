...
describe("Model/Proto", function() {
	...
	it("fabricate with options", function() {
		...
		expect(moduleProto1.mongooseSchema.statics).to.have.keys(['saveVersion', '__versioning', '__incModel', '__incField', 'returnFalse', 'sanitizeInput', 'getOne', 'getOneByID', 'getOneRaw', 'list', 'addNew', 'listAll']);
		expect(moduleProto1.mongooseSchema.methods).to.have.keys(['getName', 'getID']);
	});
	...
	describe("Model/Proto/Statics", function() {
		...
		it("getOne", function(done) {
			moduleProto.User.getOne(remember.load('item')._id, function(err, item) {
				if (err) done(err);
		...
		it("getOneByID", function(done) {
			moduleProto.User.getOneByID(remember.load('item').getID(), function(err, item) {
				if (err) done(err);
				expect(item.username).to.be.equal(savedItem.username);
				done();
			});
		...
		it("getOneRaw", function(done) {
			moduleProto.User.getOneRaw(remember.load('item')._id, function(err, item) {
				if (err) done(err);
				expect(item.username).to.be.equal(savedItem.username);
				done();
			});
		...
		it("listAll", function(done) {
			moduleProto.User.listAll(function(err, sitems) {
				if (err) done(err);
				expect(sitems).to.exist;
				expect(sitems).to.have.lengthOf(1);
				done();
		...
		afterEach(function(done) {
			moduleProto.User.remove({}, function() {
				done();
			});
		});
	});
